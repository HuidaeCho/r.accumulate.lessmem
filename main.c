...
int main(int argc, char *argv[])
{
    ...
    done = (char *)G_calloc(DONESIZE, 1);
    dir_buf = (char *)G_calloc(NIBBLESIZE, 1);
    cell_buf = Rast_allocate_c_buf();
    G_message(_("Reading direction map..."));
    for (row = 0; row < rows; row++) {
        G_percent(row, rows, 1);
        Rast_get_c_row(dir_fd, cell_buf, row);
        for (col = 0; col < cols; col++) {
            if (!Rast_is_c_null_value(&cell_buf[col]))
                SETNIBBLE(dir_buf, row, col,
                          dir_format ==
                          DIR_DEG ? cell_buf[col] / 45 : cell_buf[col]);
        }
    }
    G_percent(1, 1, 1);
    G_free(cell_buf);
