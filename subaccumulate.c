...
static void trace_down(char *, struct raster_map *, double,
                       struct Cell_head *, int, int);

void subaccumulate(struct Map_info *Map, char *dir_buf,
...
static void trace_down(char *dir_buf, struct raster_map *accum_buf,
                       double up_acc, struct Cell_head *window, int row,
                       int col)
{
    ...
    dir = GETNIBBLE(dir_buf, row, col) - 1;
    if (dir >= 0 && dir < 8)
        trace_down(dir_buf, accum_buf, up_acc, window,
