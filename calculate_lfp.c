...
static void add_table(struct Map_info *, char *, dbDriver **,
                      struct field_info **);
static int trace_up(char *, struct raster_map *, int, int, double,
                    struct headwater_list *);
...
void calculate_lfp(struct Map_info *Map, char *dir_buf,
...
static int trace_up(char *dir_buf, struct raster_map *accum_buf, int row,
                    int col, double down_length, struct headwater_list *hl)
{
    double cur_acc;
    int i, j, nup;
    ...
    for (i = -1; i <= 1; i++) {
        ...
        for (j = -1; j <= 1; j++) {
            ...
            if (GETNIBBLE(dir_buf, row + i, col + j) ==
